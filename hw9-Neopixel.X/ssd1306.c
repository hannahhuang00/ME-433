...
void ssd1306_drawChar(unsigned char x, unsigned char y, unsigned char letter) {
    ...
    for (int col=0; col<5; col++) {
        for (int row=0; row<8; row++) {
            on_or_off = ((ASCII[letter - 0x20][col]>>row) & 0b1);
            
            ssd1306_drawPixel(x+col,y+row,on_or_off);
        }    
    }
}

void ssd1306_drawString(unsigned char x, unsigned char y, char* word) {
    int i = 0;
    while (word[i] != '\0') {
        ssd1306_drawChar(x+5*i,y,word[i]);
        i++;
    }
}